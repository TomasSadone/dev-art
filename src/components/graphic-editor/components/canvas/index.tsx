...
const Canvas = () => {
    ...
    useLayoutEffect(() => {
        if (libraryWrapperRef.current) {
            ...
            const totalPadding = 32; //(px)
            canvasInstanceRef.current = initCanvas(
                smallerDimention - totalPadding
            );
        }
    }, []);
    useEffect(() => {
        fabric.Object.prototype.controls.threePointsControl.mouseUpHandler =
            onThreePointsClick;
            ...
            >
                <canvas
                    ...
                    className={`${style.canvas} ${
                        selectedItemType === 'canvas' && style.active
                    }`}
                />
                {positionThreePointsMenu && (
                    <FirstLevelThreePointsMenu
                        position={positionThreePointsMenu}
                        z={20}
                    />
                )}
            </div>
